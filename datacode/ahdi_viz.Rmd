...
# Rank/gap chart
alb_gap_data <- ahdi_tract_alb %>% 
  group_by(ahdi) %>% 
  summarise(tractnames = paste(tractnames, collapse = "\n")) %>% 
  na.exclude() 

names <- alb_gap_data$tractnames
ahdi_label <- alb_gap_data$ahdi

alb_ahdi_gap <-  ggplot(alb_gap_data, aes(x = 0, y = ahdi, label = names)) +
    geom_line(color = "grey", size = 1)+
    geom_point(aes(color=as.factor(ahdi)), size=5)+
    geom_text(label = names, hjust = 0, nudge_x = 0.005, 
              nudge_y = case_when(alb_gap_data$ahdi %in% c(6.8,8.1) ~ -0.04,
                                  alb_gap_data$ahdi %in% c(6.9,8.2) ~ 0.05,
                             .default = 0.01),
              size = 3.25, 
              # angle=55, 
              lineheight = 0.8) +
  ...
  (ahdi_alb_map + patchwork::free(alb_ahdi_gap)) +
...
# Rank/gap chart
cville_gap_data <- ahdi_tract_cville %>% 
  group_by(ahdi) %>% 
  summarise(tractnames = paste(tractnames, collapse = "\n")) %>% 
  na.exclude() 

names <- cville_gap_data$tractnames
ahdi_label <- cville_gap_data$ahdi

cville_ahdi_gap <-  ggplot(cville_gap_data, aes(x = 0, y = ahdi, label = names)) +
    geom_line(color = "grey", size = 1)+
    geom_point(aes(color=as.factor(ahdi)), size=5)+
    geom_text(label = names, hjust = 0, nudge_x = 0.005, 
              nudge_y = 0.01,
  ...
  (ahdi_cville_map + patchwork::free(cville_ahdi_gap)) +
  plot_layout(widths = c(2, 1)) +
  plot_annotation(title = 'American HD Index by Census Tract',
