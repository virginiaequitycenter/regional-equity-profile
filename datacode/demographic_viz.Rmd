---
title: "Equity Profile Demographics"
...
color_blind_pal <- c("#b2b8be","#1473af","#ede65a","#e5b039","#ea594e","#193cbc","#43daed","#b69cfd")

c("#b2b8be", "#ff641e", "#009ddc", "#fbb12d", "#b4cd23", "#ee2b75", "#2d5192", "#43daed")
...
# region_bar <- region_1790_2020 %>% 
#   mutate(text = paste0(round(percent, 0), "%")) %>% 
#   ggplot(aes(x = year, y = percent, group = year, fill = label, label = text)) +
...
cville_line <- cville_1900_2020 %>% 
  ...
  scale_y_continuous(limits = c(0,100),
                     ...
                     name = "") +
  scale_color_manual(values = c("#3B8EA5", "#b2b8be")) +
  labs(color = "", title = "White and non-white populations in Charlottesville, 1900-2020") +
  theme_minimal() +
  theme(legend.title=element_blank(),
        legend.position = "top",
        axis.text.x = element_text(angle = 45, hjust = 1),
...
cville_bar <- cville_1900_2020 %>% 
  mutate(text = paste0(round(percent, 0), "%")) %>% 
  ggplot(aes(x = year, y = round(percent, 0), group = year, fill = label, label = text)) +
...
ggarrange(cville_line, cville_bar, nrow=1)
...
ggarrange(alb_line, alb_bar, nrow=1)
...
cville_ethn_race_plot <- cville_ethn_race_2012_2022 %>% 
  ...
  scale_y_continuous(labels = label_percent(scale = 1),
                     name = "") +
  labs(color = "", title = "Charlottesville: Race/Ethnic Composition, 2012-2022") +
...
cville_ethn_race_set <- cville_ethn_race_2012_2022 %>% 
...
cville_ethn_race_set

# ggsave("images/cville_ethn_race_stacked_all.png", bg = "white")
...
cville_nativity_stacked <- cville_nativity_2022 %>% 
  ...
  labs(color = "", title = "Nativity of Charlottesville Residents, 2022") +
...
region_nativity_2022 <- region_nativity_2022 %>% 
  ...
  group_by(locality, status, total_pop, year) %>% 
  summarise(estimate = sum(estimate))

# Compute percentages
region_nativity_2022$fraction = region_nativity_2022$estimate / region_nativity_2022$total_pop
# Compute the cumulative percentages (top of each rectangle)
region_nativity_2022$ymax = cumsum(region_nativity_2022$fraction)
# Compute the bottom of each rectangle
region_nativity_2022$ymin = c(0, head(region_nativity_2022$ymax, n=-1))

# Plot the donut chart
region_nativity_chart_2022 <- ggplot(region_nativity_2022, aes(ymax = ymax, ymin = ymin, xmax = 4, xmin = 3, fill = status)) +
  geom_rect(color = "white") +
  coord_polar(theta = "y") +
  xlim(c(1.5, 4)) +
  scale_fill_manual(values = c("#8856A7", "#b2b8be")) +
...
# stacked bar
region_nativity_stacked <- region_nativity_2022 %>% 
  mutate(percent = round(estimate/total_pop *100, 0),
         text = paste0(round(percent, 0), "%"),
         status = factor(status, levels = c("US-born","Foreign-born"))) %>% 
  arrange(status) %>% 
  ...
  scale_x_continuous(labels = label_percent(scale = 1),
                     name = "") +
  scale_y_discrete(name = "") +
  labs(color = "", title = "Nativity of Charlottesville & Albemarle Residents, 2022") +
  scale_fill_manual(values = c("#b2b8be", "#8856A7")) +  
  theme_minimal() +
  theme(legend.title=element_blank(),
        legend.position = "top",
...
ggarrange(region_nativity_chart_2022, region_nativity_stacked, nrow = 1)
...
region_donut_dat <- region_foreign_born %>% 
  group_by(region) %>% 
  summarise(estimate = sum(estimate),
            total_foreign_born = first(total_foreign_born))
# Compute percentages
region_donut_dat$fraction = region_donut_dat$estimate / region_donut_dat$total_foreign_born
# Compute the cumulative percentages (top of each rectangle)
region_donut_dat$ymax = cumsum(region_donut_dat$fraction)
# Compute the bottom of each rectangle
region_donut_dat$ymin = c(0, head(region_donut_dat$ymax, n=-1))

# Plot the donut chart
region_foreign_born_donut <- region_donut_dat %>% 
  filter(round(fraction, 2) >= 0.01) %>% 
  ggplot(aes(ymax = ymax, ymin = ymin, xmax = 4, xmin = 3, fill = region)) +
  geom_rect(color = "white") +
  coord_polar(theta = "y") +
  ...
  theme(legend.title = element_blank()) +
  theme(legend.text = element_text(size = 8)) +
  labs(title = "Foreign-Born Charlottesville & Albemarle Residents, by place of birth, 2022")


# Bar chart
region_foreign_born_plot <- region_foreign_born %>% 
  mutate(percent = round(percent, 0),
         label = factor(label,
                        levels = c("Oceania",
                                   "Southern Africa","Middle Africa","Northern Africa","Eastern Africa","Western Africa",   
...
ggarrange(region_foreign_born_donut, region_foreign_born_plot, nrow = 1)
...
cville_lang_stacked <- cville_lang_all %>% 
  ...
  scale_y_discrete(name = "") +
  labs(color = "", title = "Charlottesville household languages, 2022") +
  scale_fill_manual(values = c("#b2b8be", "#8C96C6", "#6E016B")) +
  theme_minimal() +
  theme(legend.title=element_blank(),
        legend.position = "top",
...
cville_lang_stat_stacked <- cville_lang_stat %>% 
  ...
  scale_y_discrete(name = "") +
  labs(color = "", title = "Charlottesville languages by household and status, 2022") +
...
cville_dis_by_age_stacked <- cville_dis %>% 
  ...
  labs(color = "", title = "Charlottesville disability status, 2022\n (as percent of age group)") +
  theme_minimal() +
  theme(legend.title=element_blank(),
        legend.position = "top",
        panel.grid.minor.y = element_blank(),
...
ggarrange(cville_dis_age_stacked_a, cville_dis_by_age_stacked, nrow=1)
...
dis_bar <- cville_disability_cat %>%
  ...
  labs(
    ...
    title = "Charlottesville Disability Status by type for 2022"
  ) +
  theme_minimal() +
  theme(legend.position = "none")
