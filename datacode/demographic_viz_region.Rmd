---
title: "Equity Profile Demographics - Charlottesville-Albemarle Region"
...
region_line <- region_1790_2020 %>% 
  ...
  labs(color = "", title = "White and non-white populations in the Charlottesville-Region Region, 1790-2020") +
  theme_minimal() +
...
# Stacked bar
region_ethn_race_plot <- region_ethn_race_2012_2022 %>% 
  ...
  scale_y_reverse(labels = c("100%", "75%", "50%", "25%", "0%"),
                     name = "") +
  labs(color = "", title = "Charlottesville-Albemarle Region: Race/Ethnic Composition, 2012-2022") +
...
# Charlottesville-Albemarle Region (Age Groups), 2022
...
region_age_plot <- region_age_2012_2022 %>% 
  ...
  labs(fill = "", title = "Age of Charlottesville-Albemarle Region Residents, 2012 v. 2022")
...
age_plot_2012 <- region_age_2012_2022 %>% 
  filter(year == 2012) %>% 
...
age_plot_2022 <- region_age_2012_2022 %>% 
  filter(year == 2022) %>% 
  ggplot(aes(y = label, x = percent, fill = factor(year))) +  # Convert year to factor here
  geom_bar(stat = "identity", position = position_dodge2(width = 0.9, preserve = "single"), width = 0.8) +
  geom_text(aes(label = scales::percent(percent / 100, accuracy = 1)), 
...
region_age_sex_plot <- region_sex_age_2022 %>% 
  ...
  theme(legend.position = "top",
        panel.grid.minor.x = element_blank()) +
  # guides(fill = guide_legend(reverse = TRUE)) +
  labs(fill = "", title = "Sex by Age Groups for Charlottesville-Albemarle Region, 2022")
...
# Charlottesville-Albemarle Region, 2022
...
# Compute percentages
region_lang_all$fraction = region_lang_all$estimate / region_lang_all$total_households
# Compute the cumulative percentages (top of each rectangle)
region_lang_all$ymax = cumsum(region_lang_all$fraction)
# Compute the bottom of each rectangle
region_lang_all$ymin = c(0, head(region_lang_all$ymax, n=-1))

# Plot the donut chart
region_lang_donut <- region_lang_all %>%
  ...
  theme(legend.title = element_blank()) +
  theme(legend.text = element_text(size = 8)) +
  labs(title = "Charlottesville-Albemarle Region household languages, 2022")

# region_lang_donut

# stacked bar
region_lang_stacked <- region_lang_all %>% 
  mutate(text = paste0(round(percent, 0), "%\n", scales::comma(estimate), " households"),
  ...
  scale_y_discrete(name = "") +
  labs(color = "", title = "Charlottesville-Albemarle Region household languages, 2022") +
  scale_fill_manual(values = c("#b2b8be", "#8C96C6", "#6E016B")) +
  theme_minimal() +
  theme(legend.title=element_blank(),
        legend.position = "top",
...
region_lang_stat <- region_lang_stat %>%
  ...
  select(locality, estimate, total_non_eng, group, status, year, total_households)

# stacked bar
region_lang_stat_stacked <- region_lang_stat %>% 
  ...
  scale_y_discrete(name = "") +
  labs(color = "", title = "Charlottesville-Albemarle Region languages by household and status, 2022") +
...
region_dis_age_stacked_a <- region_disability_dat %>% 
  ...
  labs(color = "", title = "Charlottesville-Albemarle Region disability status by age, 2022\n(as percent of total population)") +
  theme_minimal() +
  theme(legend.title=element_blank(),
        legend.position = "top",
        panel.grid.minor.y = element_blank(),
...
region_dis_by_age_stacked <- region_dis %>% 
  ...
  labs(color = "", title = "Charlottesville-Albemarle Region disability status, 2022\n (as percent of age group)") +
  theme_minimal() +
  theme(legend.title=element_blank(),
        legend.position = "top",
        panel.grid.minor.y = element_blank(),
...
region_dis_by_age_stacked

# ggarrange(region_dis_age_stacked_a, region_dis_by_age_stacked, nrow=1)
...
dis_donut <- region_disability_cat %>%
...
ggplot(aes(x = 2, y = percent, fill = label)) +
  ...
  labs(fill = "Category",title = "Charlottesville-Albemarle Region Disability Status by type for 2022") +
  theme_void() +
  theme(legend.position = "none")

# bar chart
dis_bar <- region_disability_cat %>%
  mutate(label = factor(label, levels = c("Vision\nDifficulty", "Self-Care\nDifficulty", "Hearing\nDifficulty", "Cognitive\nDifficulty", "Independent Living\nDifficulty", "Ambulatory\nDifficulty"))) %>%
  ...
  labs(
    ...
    title = "Charlottesville-Albemarle Region Disability Status by type for 2022"
  ) +
  theme_minimal() +
  theme(legend.position = "none")
