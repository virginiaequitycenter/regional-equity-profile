...
```
```{r, echo=FALSE, fig.width=14, fig.height=9.5, fig.cap=""}
...
selection <- ahdi_tract_alb %>% filter(row_num == 29)
d <- data.frame(lat = c(37.821127), lon = c(-78.596546))
...
ahdi_alb_map <- ahdi_tract_alb %>%
  ...
  geom_sf(data = selection, color = "blue", fill = NA, linewidth = 0.75) +
  geom_point(data = d, aes(x = lon, y = lat), size = 5, color = "firebrick") +
  geom_text(data = d, aes(x = lon, y = lat), label = "Yancey Community Center", size = 3.5, color = "black", fontface = "bold", 
               # nudge_y = 0.001, 
               nudge_x = 0.1
               ) +
  geom_sf_text(aes(label = row_num), size = 3.5, 
               # nudge_y = 0.001, 
               nudge_y = case_when(ahdi_tract_alb$row_num == 29 ~ 0.02,
                                   .default = 0),
               color = case_when(ahdi_tract_alb$row_num == 29 ~ "blue",
                                   .default = "black"),
              fontface = case_when(ahdi_tract_alb$row_num == 29 ~ "bold",
                                   .default = "plain")
               ) +
  theme_map()+
  theme(
