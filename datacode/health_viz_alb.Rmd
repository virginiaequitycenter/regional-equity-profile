...
```{r, echo=FALSE, message=FALSE, fig.width=10, fig.height=8.75}
...
alb_insured <- alb_health_insured_tract %>% 
  ...
  summarise(tractnames = paste(tractnames, collapse = "; ")) %>% 
  # mutate(tractnames = case_when(per_uninsured_rounded == 1 ~ "Earlysville; Polo Grounds Road;\nBranchland; Crozet-Beaver Creek;\nWestern Ridge-Westhall",
  #                               per_uninsured_rounded == 1 ~ 
  #                               .default = tractnames)) %>% 
  na.exclude() 
...
alb_insured_gap <-  ggplot(alb_insured, aes(x = 0, y = per_uninsured_rounded, label = names)) +
    ...
    geom_text(label = str_wrap(names, width = 50), hjust = 0, nudge_x = 0.005, 
              nudge_y = case_when(alb_insured$per_uninsured_rounded %in% c(5,6) ~ 0.1,
