...
chr_locality %>% 
  ...
  scale_x_continuous(breaks = seq(from = 2019, to = 2024, by = 1),
                     name = "") +
  scale_y_continuous(breaks = seq(from = 75, to = 85, by = 5),
                     limits = c(75,85),
...
```{r health-insurance-time, fig.width = 9.5, fig.height = 6, fig.retina = 2, fig.cap=""}
...
region_health_insured_county_race_2017_2022 %>% 
  ...
  # scale_color_manual(values = c("#3B8EA5")) +
  labs(color = "",
       title = "Residents with No Health Insurance: 2017-2022", 
       subtitle = locality_name,
       caption = source_acs) +
  theme_minimal() +
  theme(legend.title=element_blank(),
...
chr_parks %>% 
  # filter(region == "Region") %>% 
  na.omit() %>% 
  ggplot(aes(x = year, y = round(exer_access_per,0), label = paste0(round(exer_access_per,0), "%"))) +
  stat_xspline(size=1) +
  geom_point() +
  geom_text(size = 5, hjust=0.5, vjust=-1.15, color="black") +
  ...
  scale_y_continuous(limits = c(0,100),
                     # labels = scales::label_currency(scale = 1),
                     name = "Percent of exercise opportunities") +
  # scale_color_manual(values = c("#3B8EA5")) +
  facet_wrap(~region, scales = "free", ncol = 2) +
  labs(color = "",
...
region_commute_dat %>% 
  ggplot(aes(x = total_ages_percent, y = as.character(year), group=as.character(year), fill = status)) +
...
region_tenure %>% 
  ggplot(aes(x = percent, y = locality, group=locality, fill = label)) +
  geom_bar(stat = "identity") +
  geom_label_repel(aes(label = case_when(
...
```
### Evictions
```{r evictions, fig.width=8, fig.height=9}
...
evictions %>% 
  mutate(year_chr = case_when(filed_year == 2024 ~ "2024*",
                              .default = as.character(filed_year))) %>% 
  ggplot(aes(x = year_chr, y = value, color = name, group = name, label = value)) +
  stat_xspline(size=1) +
  geom_point() +
  geom_text(size = 5, hjust=0.5, 
            vjust=-1,
  ...
  labs(color = "",
       ...
       subtitle = "2018-2024",
       caption = "*2024-to-date includes data through September 2024") +
  theme_minimal() +
  theme(legend.title=element_blank(),
        legend.position = "top",
        panel.grid.minor = element_blank(),
...
```{r pit_count, fig.width = 9.5, fig.height = 6, fig.retina = 2, fig.cap=""}
pitcount_2007_2023 <- read_csv("../supplemental_downloadsremoved/data/pitcount_2007_2023.csv")
pitcount_2007_2023 <- pitcount_2007_2023 %>% 
...
pitcount_2007_2023 %>% 
  ...
  annotate("text", x = 2012, y = 100, size = 3.5, hjust = 0, color = "#333333",
           label =
"Opening of The Crossings
(permanent supportive housing)") +
  geom_vline(xintercept = 2022, linetype = 2, color = "#333333") +
  annotate("text", x = 2022, y = 275, size = 3.5, hjust = 0, color = "#333333",
           label =
...
and lack of affordable housing") +
  ...
  scale_x_continuous(breaks = seq(from = 2007, to = 2023, by = 1),
                     name = "") +
  labs(color = "",
       title = "PIT Count", 
       subtitle = locality_name,
...
```{r hic_count, fig.width = 9.5, fig.height = 6, fig.retina = 2, fig.cap=""}
hiccount_2007_2023 <- read_csv("../supplemental_downloadsremoved/data/hiccount_2007_2023.csv")
