---
title: "Albemarle Inclusive Community Profile"
# output:
...
```{r insurance-tract, fig.width=12, fig.height=10.25, fig.cap="Residents with No Health Insurance by Census Tract. The percent of residents without health insurance ranges from lows between 0%-4% in many tracts to a high of 31% in the Albemarle HS area."}
...
alb_insured_gap <-  ggplot(alb_insured, aes(x = 0, y = per_uninsured_rounded, label = names)) +
  ...
  (alb_insured_map + patchwork::free(alb_insured_gap)) +
  plot_layout(widths = c(1.75, 1)) +
...
```{r alice-threshold, fig.width = 9.5, fig.height = 6, fig.retina = 2, fig.cap="ALICE Thresholds 2010-2023. The ALICE threshold for households under 65 years old was $75,000 in 2023 and $65,000 for 65 years and older."}
# ALICE thresholds: Albemarle ----
alb_ALICE_threshold <- read_csv("../data/alb_ALICE_threshold_2010_2023.csv")
...
alb_ALICE_threshold %>% 
  ...
  scale_y_continuous(labels = label_currency(scale = 1, scale_cut = cut_short_scale()),
    ...
    guides(color = guide_legend(reverse = TRUE)) +
labs(color = "", 
       title = "ALICE Thresholds 2010-2023", 
      subtitle = locality_name, 
      caption = "Data Sources: ALICE Threshold, 2010–2023; U.S. Census Bureau, American Community Survey, 2010–2023") +  
  theme_minimal() +
   theme(legend.position = "top",
        legend.justification.top = "left",
        legend.title=element_blank(),
...
```{r alice-race, fig.width = 9.5, fig.height = 6, fig.retina = 2, fig.cap="ALICE Households by Race/Ethnicity, 2023."}
# ALICE households by race/ethnicity: Albemarle, 2022 ----
alb_ALICE_households_by_race <- read_csv("../data/alb_ALICE_households_by_race_2023.csv")
...
alb_ALICE_households_by_race %>% 
  ...
  labs(color = "", 
      ...
      subtitle = locality_name, 
      caption = "Data Sources: ALICE Threshold, 2023; U.S. Census Bureau, American Community Survey, 2023") +
  theme_minimal() +
 theme(legend.position = "top",
        legend.location = "plot",
        legend.title=element_blank(),
...
```{r alice-district, fig.width = 9.5, fig.height = 6, fig.retina = 2, fig.cap="ALICE Households by District, 2023."}
# ALICE households by District: Albemarle ----
alb_ALICE_households_district <- read_csv("../data/alb_ALICE_households_district_2023.csv")

alb_ALICE_households_district <- alb_ALICE_households_district %>%
  rename(GEOID = geo_id2, locality = county, number = value, ) %>% 
  mutate(level = case_when(name == "above_alice_households" ~ "above",
                           name == "alice_households" ~ "alice",
                           name == "poverty_households" ~ "poverty",
                           .default = name),
         group = str_remove(geo_display_label, ", Albemarle County, Virginia")) %>% 
  select(GEOID, locality, level, group, number, households, percent, year)

alb_ALICE_all <- alb_ALICE_households_by_race %>% 
  filter(group == "All")

alb_ALICE_households_district <- rbind(alb_ALICE_households_district, alb_ALICE_all)

alb_ALICE_households_district <- alb_ALICE_households_district[order(alb_ALICE_households_district$level),]

alb_ALICE_households_district %>% 
  arrange(desc(level)) %>% 
  mutate(group = factor(group,
                       levels = c("White Hall district", "Scottsville district", "Samuel Miller district", "Rivanna district", "Rio district", "Jack Jouett district", "All"),
                       labels = c("White Hall District", "Scottsville District", "Samuel Miller District", "Rivanna District", "Rio District", "Jack Jouett District", "Albemarle County")),
         level = factor(level, levels = c("above", "alice", "poverty"),
                        labels = c("Above ALICE", "ALICE", "Poverty")),
  text = case_when(percent >= 1 ~ paste0(round(percent, 0), "%"),
                   percent < 1 ~ paste0(round(percent, 1), "%"))) %>%
  ...
  coord_flip() +
  labs(color = "", 
       title = "ALICE Households by District", 
      subtitle = locality_name, 
      caption = "Data Sources: ALICE Threshold, 2023; U.S. Census Bureau, American Community Survey, 2023") +
  theme_minimal() +
 theme(legend.position = "top",
...
```{r rent-burden-tract, fig.height=10.5, fig.width=12}
alb_rent_burden_tract <- read_csv("../data/alb_rent_burden_tract_2023.csv") %>%
  mutate(GEOID = as.character(GEOID),
         level_combined = case_when(level %in% c("Burdened", "Severely Burdened") ~ "Burdened",
                                    .default = level)) %>% 
  group_by(GEOID, locality, tract, tractnames, level_combined) %>% 
  summarise(estimate = sum(estimate),
            total_units = first(total_units)) %>% 
  ungroup() %>% 
  mutate(percent = round(estimate/total_units *100, 0),
         text_color = case_when(percent >= 65 ~ "white",
                                percent <65 ~ "black"))
...
# Geospatial visualization
alb_rent_burden_map <- alb_rent_burden_tract %>%
  filter(level_combined == "Burdened") %>%
  mutate(text = paste0(round(percent,0), "%")) %>%
  ggplot() +
  annotation_map_tile(zoomin = 1, progress = "none", cachedir = "../data/tempdata/") +
  geom_sf(aes(fill = percent), color = "white") +
  ...
  scale_fill_stepsn(colors = pal_inc,
                    labels = scales::label_percent(scale = 1),
                    n.breaks = 7,
                    guide = guide_colourbar(title = "Percent Rent Burdened",
                                            title.position = "top",
                                            direction = "horizontal",
                                            title.hjust = 0.5,
                                            # nbin = 6,
...
# Rank plot
alb_rent_burden_data <- alb_rent_burden_tract %>% 
  filter(level_combined == "Burdened") %>%
  group_by(percent) %>% 
  summarise(tractnames = paste(tractnames, collapse = "; ")) %>% 
  na.exclude() 

names <- alb_rent_burden_data$tractnames
homeowner <- alb_rent_burden_data$percent

alb_rent_burden_data_rank <- ggplot(alb_rent_burden_data, aes(x = 0, y = percent, label = names)) +
    geom_line(color = "grey", linewidth = 1)+
    geom_point(aes(color=as.factor(percent)), size=5)+
    geom_text(label = str_wrap(names, width = 48), hjust = 0, nudge_x = 0.006, 
              nudge_y = case_when(alb_rent_burden_data$percent %in% c(35,36,46,47,67) ~ 0.3,
                                  alb_rent_burden_data$percent %in% c(34,44,66) ~ -0.3,
                             .default = 0.01),
              size = 4,
              lineheight = 0.7) +
   scale_color_carto_d(name = "Percent Rent Burdened: ",
                           type = "diverging", palette = "Emrld", direction = 1) +
    scale_x_continuous(limits = c(0,.1),
                       expand = c(0.03, 0.003))+
    scale_y_continuous(
  ...
  (alb_rent_burden_map + patchwork::free(alb_rent_burden_data_rank)) +
  plot_layout(widths = c(1.75, 1)) +
  plot_annotation(title = "Rent Burdened Households by Census Tract (30% or more household income spent on housing)", 
      subtitle = locality_name,
      caption = source_acs,
      theme = theme(plot.title = element_text(size = 18),
                    plot.subtitle = element_text(size = 16),
